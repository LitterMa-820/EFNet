    ...
    try:
        for i, (images, gts, edges) in enumerate(train_loader, start=1):
            ...
            if i % 20 == 0 or i == total_step or i == 1:
                print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Total_loss: {:.4f} loss_pred: {:.4f} loss_e: {:0.4f}'.
                      format(datetime.now(), epoch, opt.epoch, i, total_step, loss.data, loss_pred.data, loss_e.data))
                logging.info(
                    '[Train Info]:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Total_loss: {:.4f} loss_pred: {:.4f} loss_e: {:0.4f}'.
                    format(epoch, opt.epoch, i, total_step, loss.data, loss_pred.data, loss_e.data))
                # TensorboardX-Loss
                writer.add_scalars('Loss_Statistics',
                                   {'loss_pred': loss_pred.data, 'loss_e': loss_e.data,
